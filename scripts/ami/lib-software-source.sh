...
ironlog_source_config="${IRONLOG_BUILD_SOURCE_CONFIG:-${IRONLOG_IRONLOG_ETC_DIR:-/etc/ironlog}/build-source.conf}"
...
ironlog_dnf() {
  if [ "${IRONLOG_SOFTWARE_SOURCE:?}" = bundle ]; then
    dnf --disablerepo='*' --enablerepo=ironlog-bundle --setopt=plugins=0 --setopt=gpgcheck=1 --setopt=repo_gpgcheck=0 "$@"
  else
    dnf "$@"
  fi
}
